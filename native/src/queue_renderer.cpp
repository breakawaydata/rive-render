...
QueueRenderResult renderWithQueue(const Config& config, const std::vector<uint8_t>& rivBytes)
{
    ...
    try
    {
        ...
        // Post-warmup pass. By now all nested artboards exist and state
        // machine transitions have settled. This callback:
        // 1. Downloads CDN-hosted fonts (e.g. Archivo) and decodes them.
        //    Font assets must be decoded AFTER artboard instantiation so
        //    TextStyle fileAssetReferencers get dirtied.
        // 2. Re-binds the VM with the file's primary ViewModel so nested
        //    artboard text runs receive property values.
        // 3. Advances twice to propagate data binds through nested artboards.
        {
